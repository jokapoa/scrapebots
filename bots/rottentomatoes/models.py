...
class RottenTomatoesSpider(object):
    ...
    def __init__(self, url, category):
        ...
        self.url = url
        self.code = self.url[url.index("/" + category + "/") + len("/" + category + "/"):]
...
class RottenTomatoesMovies(RottenTomatoesSpider):
    ...
    def __init__(self, url):
        ...
        self.title = self.soup.find_all("h1", {"id": "movie-title"})[0].text.strip()  # parse source to get title
        self.tmeter_info = {
            "score": "Not found.",
    ...
    def get_score(self):
        ...
        if not self.is_updated[0]:
            try:
                tmeter_info = {
                    "score": int(self.soup.find_all("span", {"class": "meter-value superPageFontColor"})
                                 [0].text.replace("%", "").strip()),
                    "avg_rating": float(self.soup.find_all("div", {"class": "superPageFontColor"})
                                        [0].text.replace("Average Rating: ", "").strip().replace("/10", "").strip()),
                    "reviews": int(self.soup.find_all("div", {"class": "superPageFontColor"})
                                   [1].text.replace("Reviews Counted: ", "").strip())
                }  # critics
            except:
                tmeter_info = {
                    "score": "Not found.",
            ...
            try:
                audience_meter_info = {
                    "score": int(
                        self.soup.find_all("span", {"class": "superPageFontColor", "style": "vertical-align:top"})
                        [0].text.replace("%", "").strip()),
                    "avg_rating": float(
                        self.soup.find_all("div", {"class": "audience-info hidden-xs superPageFontColor"})
                        [0].find_all("div")
                        [0].text.replace("Average Rating:", "").strip().replace("/5", "").strip()),
                    "reviews": int(self.soup.find_all("div", {"class": "audience-info hidden-xs superPageFontColor"})
                                   [0].find_all("div")
                                   [1].text.replace("User Ratings:", "").replace(",", "").strip())
                }  # audience
            except:
                audience_meter_info = {
                    "score": "Not found.",
    ...
    def get_critics_review(self):
        ...
        if not self.is_updated[1]:
            try:
                critics_review = str(self.soup.find_all("p", {"class": "critic_consensus superPageFontColor"})
                                     [0].text.replace("Critics Consensus:", "").strip())
            except:
    ...
    def get_movie_info(self):
        ...
        def get_panel_info(key, soup):
            ...
            assert (key in ["Rating", "Genre", "Directed By", "Written By", "On DVD", "In Theaters", "Box Office",
                            "Runtime", "Studio"])

            panel = soup.find_all("div", {"class": "info"})[0].find_all("div")
            try:
                index = panel.index([x for x in panel if key in x.text][0]) + 1  # get index of value

                if key == "On DVD" or key == "In Theaters":
                    value = datetime.strptime(panel[index].text
                                              .replace("&nbsp;Wide", "")
                                              .replace("&nbsp;wide", "")
                                              .replace("wide", "").strip(), "%b %d, %Y")
                elif key == "Box Office" or key == "Runtime":
                    value = panel[index].text.strip().replace("$", "").replace(",", "").replace(" minutes", "")
                    if key == "Box Office":  # value is a price
                        value = value[:value.index(".")]  # remove decimal points
                    value = int(value)
                else:  # rating, genre, directed by, written by, studio
                    value = [x.strip() for x in panel[index].text.strip().split(",")]
            except:
                value = "Not found."
        ...
        if not self.is_updated[2]:
            movie_info = {
                "plot": self.soup.find_all("div", {"id": "movieSynopsis"})[0].text.strip(),
                ...
                "cast": [x.text.strip() for x in
                         self.soup.find_all("div", {"class": "cast-item media inlineBlock "})] +
                        [x.text.strip() for x in
                         self.soup.find_all("div", {"class": "cast-item media inlineBlock  castRemaining"})]
            }
